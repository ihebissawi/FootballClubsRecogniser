...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            if (idx == self.selectedSegmentIndex && self.selectedTextFont){
                textFont = self.selectedTextFont;
            }

            CGFloat stringWidth = 0;
            CGFloat stringHeight = 0;
            if([titleString respondsToSelector:@selector(sizeWithAttributes:)]) {
                stringWidth = [titleString sizeWithAttributes:@{NSFontAttributeName: textFont}].width;
                stringHeight = [titleString sizeWithAttributes:@{NSFontAttributeName: textFont}].height;
            }
            else {
                stringWidth = roundf([titleString sizeWithFont:textFont].width);
                stringHeight = roundf([titleString sizeWithFont:textFont].height);
            }
            ...
            titleLayer.font = (__bridge CFTypeRef)(textFont.fontName);
